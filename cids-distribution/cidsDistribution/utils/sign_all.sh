...
if [[ -f ${CIDS_DISTRIBUTION_DIR}/.private/keystore && ${CIDS_DISTRIBUTION_DIR}/.private/keystore.pwd ]]; then
    ...
    find -L ${MAVEN_LIB_DIR} -name *.jar -type f -not -path "${MAVEN_LIB_DIR}/org/apache/maven/*" -not -path "${MAVEN_LIB_DIR}/org/sonatype/*" -not -path "${MAVEN_LIB_DIR}/org/codehaus/plexus/*" -newermm ${MAVEN_LIB_DIR}/.signed -exec ${CIDS_DISTRIBUTION_DIR}/utils/sign.sh ${CIDS_DISTRIBUTION_DIR}/.private/keystore `cat ${CIDS_DISTRIBUTION_DIR}/.private/keystore.pwd` {} \;
